...
class WrapperDB:    
    ...
    def elencoPost(self, as_dict = False):
        ...
        try:
            ...
            sql = "SELECT Id, Autore, Testo, [Like] FROM SOLD_FB_Post ORDER BY [Like] DESC"
            cur.execute(sql)
            lista = cur.fetchall()
        except:
    ...
    def inserisciPost(self, parametri):
        ...
        try:
            c = self.connetti() 
            cursore = c.cursor()
            sql = "INSERT INTO SOLD_FB_Post (Autore, Testo) VALUES (%s , %s)"
    ...
    def inserisciPostSP(self, parametri):
        ...
        try:
            ...
            c = self.connetti() 
            cursore = c.cursor()
            res = cursore.callproc('dbo.SOLD_InserisciPost', parametri)
            c.commit()
            #print("INSERIMENTO POST AVVENUTO")
            #return True            
            return res[2]
    ...
    def daiLikeAPost(self, id, is_like = True):
        ...
        try:
            c = self.connetti() 
            cursore = c.cursor()
            sql = "UPDATE SOLD_FB_Post SET [Like] = "
            if is_like == True: 
                sql += "[Like] + 1 "
            else:
                sql += "[Like] - 1 "
    ...
    def eliminaPost(self, id):
        ...
        try:
            c = self.connetti() 
            cursore = c.cursor()
            sql = "DELETE SOLD_FB_Post WHERE id = %d"
            cursore.execute(sql, id)
            c.commit()
    ...
    def aggiungiCommento(self, parametri):
        try:
            c = self.connetti() 
            cursore = c.cursor()
            sql = "INSERT INTO SOLD_FB_Commenti VALUES (%d, %s, %s)"
            cursore.execute(sql, parametri)
            c.commit()
            self.disconnetti(c)
            return True
    ...
    def modificaCommento(self, parametri):
        try:
            c = self.connetti() 
            cursore = c.cursor()
            sql = "UPDATE SOLD_FB_Commenti SET Autore = %s, Testo = %s WHERE idPost = %d"     
            cursore.execute(sql, parametri)
            c.commit()
            self.disconnetti(c)
            return True
    ...
    def eliminaCommento(self, parametro):
        try:
            c = self.connetti() 
            cursore = c.cursor()
            sql = "DELETE FROM SOLD_FB_Commenti WHERE IdPost = %d"        
            cursore.execute(sql, parametro)
            c.commit()
            self.disconnetti(c)
            return True
